...
class Generator {
    companion object {
        ...
        fun genRepository(tableInfo: TableInfo, generateInfo: GenerateInfo): String {
            ...
            val content: String
            repositoryCode.append("package $parentPackageName.repositories\n")
                    ...
                    .append("\n")
                    .append("interface ${klassName}Repository : CrudRepository<$klassName, Long>{\n")
        ...
        fun genService(tableInfo: TableInfo, generateInfo: GenerateInfo): String {
            val parentPackageName = generateInfo.parentPackageName
            val packageName = "${parentPackageName}.services"
            val tableName = tableInfo.tableName
            val klassName = StringUtils.toUpperCase(tableName)
            val repositoryCode = StringBuffer()
            val fileName: String =
                    (generateInfo.projectPath ?: "") +
                            "/src/main/kotlin/${StringUtils.toOtherDelimiter(packageName, '.', '/')}/${StringUtils.toUpperCase(tableInfo.tableName)}Service.kt"
            val content: String
        ...
        fun genServiceImpl(tableInfo: TableInfo, generateInfo: GenerateInfo): String {
            val parentPackageName = generateInfo.parentPackageName
            val packageName = "${parentPackageName}.services.impl"
            val tableName = tableInfo.tableName
            val klassName = StringUtils.toUpperCase(tableName)
            val repositoryCode = StringBuffer()
            val fileName: String =
                    (generateInfo.projectPath ?: "") +
                            "/src/main/kotlin/${StringUtils.toOtherDelimiter(packageName, '.', '/')}/${klassName}ServiceImpl.kt"
            val content: String
            repositoryCode.append("package $packageName\n\n")
                    .append("import com.learnwy.system.commom.ResquestPage\n")
                    .append("import $parentPackageName.dto.$klassName\n")
                    .append("import $parentPackageName.repositories.${klassName}Repository\n")
                    .append("import $parentPackageName.services.BaseServiceQuery\n")
                    ...
                    .append("@Service\n")
                    .append("class ${klassName}ServiceImpl(@Autowired override val repository: ${klassName}Repository) : ${klassName}Service, BaseServiceQuery<$klassName, Long> {\n")
                    .append("\toverride fun selectAll(page: ResquestPage): List<$klassName> {\n")
                    .append("\t\tval pageRequest: PageRequest = PageRequest.of(page.page, page.pageSize)\n")
                    .append("\t\tval pages: Page<$klassName> = repository.findAll(pageRequest)\n")
                    .append("\t\tpage.totalPages = pages.totalPages\n")
                    .append("\t\tpage.total = pages.numberOfElements\n")
                    .append("\t\treturn pages.content\n")
                    .append("\t}\n")
                    .append("}")
            content = repositoryCode.toString()
            if (FileUtils.createFileIfNoExists(fileName)) {
        ...
        fun genController(tableInfo: TableInfo, generateInfo: GenerateInfo): String {
            ...
            repositoryCode.append("package $packageName\n\n")
                    ...
                    .append("""@RequestMapping(path = arrayOf("/system/$tableName"))""")
